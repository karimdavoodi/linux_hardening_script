#!/bin/bash
#      
# Title         :  Hardening script for Linux (branch of compliance.sh)
# Date          :  2006
# Version       :  0.1.1    
#      
...
#-----------------------------------------
server=1
s_net_m="{can, 9p, wimax, wireless, mac80211, ieee802154, bluetooth}"
d_net_m="{can, 9p, phonet}"
...
#
#-----------------------------------------
# In comparision with default settings we've had some changes:
# Desktop user needs printer: 		cups    enable
# Desktop user needs file sharing: 	smb	enable
#-----------------------------------------
...
#-----------------------------------------
# In comparision with default settings we've had some changes:
# Server user needs file sharing:      		smb     enable
# Server user needs ......
#-----------------------------------------
...
Disable_Groups_list=( games uucp )
...
FileSystemChecks() {
		...
		Warning "/tmp not exists.."
		 ...
		 if [[ $(mount | grep "/tmp" | awk '{print $NF}') = '(rw,noexec,nosuid,nodev)' ]]; then
                       ...
                       Exists "Partition /tmp $(mount | grep "/tmp" | awk '{print $NF}')"
                        ...
                        awk '1;/\/tmp/{$4="nosuid,nodev,noexec";print}' $Fstab | column -t > /tmp/1
						mv /tmp/1 $Fstab
	...
	if ! $( mount | grep -q "/dev/shm" ); then
		...
		if [[ $(mount | grep "/dev/shm" | awk '{print $NF}') = '(rw,noexec,nosuid,nodev)' ]]; then
			...
			Fstab='/etc/fstab'
			cp ${Fstab}{,-bkp-$(date +%F)} >/dev/null 2>&1	
			awk '1;/\/dev\/shm/{$4="nosuid,nodev,noexec";print}' $Fstab | column -t > /tmp/1
			mv /tmp/1 $Fstab
...
#+++++++++++++++
#This if else clause prevents enforcing Kernel Tuninig, Why?
#+++++++++++++++
...
Disable_user(){
	Tmp=$(mktemp)
	if [ "$server" == "1" ]; then
		Disable_Users_list_S > "${Tmp}"
	else
		Disable_Users_list_D > "${Tmp}"
	fi
	PassFile='/etc/passwd'
...
Service_Control(){
	...
	ServiceList="$(mktemp)"
	if [ "$server" == "1" ]; then
		SystemServiceList_S > $ServiceList
	else
		SystemServiceList_D > $ServiceList
	fi
	local srv=""
	local option=""
...
Xinetd_srv_control(){
	...
	Tmp=$(mktemp)
	if [ "$server" == "1" ]; then
		Xinetd_srv_list_S > $Tmp
	else
		Xinetd_srv_list_D > $Tmp
	fi
	local srv=""
	local option=""
	while read -u 3 srv option
...
Kernel_Module_Disable(){
	Message "Disable Unused Network Kernel Modules..."
	if [ "$server" == "1" ]; then
			rm -rf /lib/modules/`uname -r`/kernel/net/$s_net_m
	else
			rm -rf /lib/modules/`uname -r`/kernel/net/$d_net_m
	fi
}


#------------------------{ Main Program}-----------------------------------------
Message "Start of Hardenig ========================================================"
if [ "`grep Desktop /etc/issue`" == "" ]; then 
	server=1
	Message "Hardening 'Server' "
else 
	server=0
	Message "Hardening 'Desktop' "
fi
exit 0
SSH_Security_check
TCPWrapper_check
...
Xinetd_srv_control
Kernel_Module_Disable
tar -czf /etc/hardening_backup.tar.gz /etc/*-bkp-*
rm -f  /etc/*-bkp-*
Message "End of Hardenig==========================================================="
